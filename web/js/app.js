/* Orchestrateur StopAddict (ES module) — aligné “monolithe” */
...
let Settings = null;
try { const m = await import('./settings.js'); if (typeof m?.mountSettings==="function") Settings = m; } catch {}
...
let Resources = null;
try { const m = await import('./resources.js'); Resources = m; } catch {}
...
const dbg = {
  push(msg, type = "info") {
    ...
    if (S.debug.logs.length > 600) S.debug.logs.shift();
...
function initAgeGate() {
  ...
  // Lien ressources présent dans index.html (fonctionne même sans module)
  $("#age-resources")?.addEventListener("click", (e)=>{
    e.preventDefault();
    const d = $("#resources-dialog");
    if (d?.showModal) d.showModal(); else d?.classList.remove("hide");
  });
...
function reflectCounters(){
  ...
  // On désactive UNIQUEMENT les boutons +/- pour pouvoir re-cocher depuis l’accueil
  const setRowDisabled=(wrapSel,on)=>{
    const wrap = $(wrapSel);
    if (!wrap) return;
  ...
  setRowDisabled("#ctr-cigs .ctr-row", !(S.today.active.cigs && S.modules.cigs));
  setRowDisabled("#ctr-joints .ctr-row", !(S.today.active.joints && S.modules.joints));
  setRowDisabled("#ctr-beer .ctr-row",   !(S.today.active.beer && S.modules.beer));
  setRowDisabled("#ctr-hard .ctr-row",   !(S.today.active.hard && S.modules.hard));
  setRowDisabled("#ctr-liqueur .ctr-row",!(S.today.active.liqueur && S.modules.liqueur));
}
...
function initCounters(){
  ...
  for(const k of KINDS){
    ...
    el.addEventListener("change", ()=>{
      S.today.active[k]=el.checked; reflectCounters(); saveState(S);
...
function renderChart(period=_period){
  ...
  const canvas=$("#chart-main"); if(!canvas||typeof Chart==="undefined") return;
  ...
  if(period==="day"){
    ...
    counters={cigs:d.cigs||0,joints:d.joints||0,beer:d.beer||0,hard:d.hard||0,liqueur:d.liqueur||0,cost:d.cost||0,saved:d.saved||0};
    ...
    for(const k of Object.keys(S.history)){
      if(k>=range.start && k<=range.end){
        const d=S.history[k]||{};
        counters.cigs+=d.cigs||0; counters.joints+=d.joints||0; counters.beer+=d.beer||0;
        counters.hard+=d.hard||0; counters.liqueur+=d.liqueur||0; counters.cost+=d.cost||0; counters.saved+=d.saved||0;
      }
    }
  ...
  chart=new Chart(canvas.getContext("2d"),{
    ...
    options:{
      ...
      plugins:{ legend:{ display:false },
        tooltip:{ callbacks:{ label:(ctx)=> ctx.dataIndex>=5
...
function initStats(){
  ...
  $("#file-import-json")?.addEventListener("change", async (ev)=>{
    ...
    try{
      ...
      S={...DefaultState(),...obj};
      saveState(S); hydrateUIFromState(); renderChart(_period); Cal?.update?.(S);
      dbg.push("Import JSON ok","ok");
    }catch(e){
      dbg.push("Import JSON erreur: "+e?.message,"err"); alert("Import JSON invalide.");
    }finally{
      ev.target.value="";
    }
  });
...
function initHabits(){
  setVal("#goal-cigs",S.goals.cigs); setVal("#goal-joints",S.goals.joints); setVal("#goal-beer",S.goals.beer);
  setVal("#goal-hard",S.goals.hard); setVal("#goal-liqueur",S.goals.liqueur);
  ...
  $("#btn-habits-save")?.addEventListener("click", ()=>{
    ...
    persistTodayIntoHistory(); updateHeader(); renderChart(_period); saveState(S); Cal?.update?.(S);
    dbg.push("Objectifs quotidiens enregistrés","ok");
  });

  $("#btn-habits-reset")?.addEventListener("click", ()=>{
    S.goals={...DefaultState().goals};
    setVal("#goal-cigs",0); setVal("#goal-joints",0); setVal("#goal-beer",0); setVal("#goal-hard",0); setVal("#goal-liqueur",0);
    persistTodayIntoHistory(); updateHeader(); renderChart(_period); saveState(S); Cal?.update?.(S);
    dbg.push("Objectifs réinitialisés","ok");
  });

  // Dates clés minimal (si présents dans l’index)
  const D=S.dates; const map=[
...
/* ---------- i18n (hook) ---------- */
async function initI18nIfAvailable(){ /* placeholder */ }
...
/* ---------- Règle “Alcool global” vs sous-modules ---------- */
function wireAlcoholRule(){
  const mGlob = $("#mod-alcohol");
  const mBeer = $("#mod-beer");
  const mHard = $("#mod-hard");
  const mLiq  = $("#mod-liqueur");
  if (!mGlob || !mBeer || !mHard || !mLiq) return;

  const apply = ()=>{
    if (mGlob.checked){
      mBeer.checked = false; mHard.checked = false; mLiq.checked = false;
      S.modules.alcohol = true; S.modules.beer=false; S.modules.hard=false; S.modules.liqueur=false;
    } else {
      S.modules.alcohol = false;
    }
    saveState(S); reflectCounters(); Cal?.update?.(S);
  };
  mGlob.addEventListener("change", apply);
}
...
(async function initApp(){
  ...
  if (Settings?.mountSettings){
    Settings.mountSettings({ S, DefaultState, saveState, persistTodayIntoHistory, updateHeader, renderChart, reflectCounters, dbg });
  }
  ...
  // Debug overlay (5 clics sur la date)
  let taps=[]; $("#today-date")?.addEventListener("click", ()=>{
    const now=Date.now(); taps=taps.filter(t=> now-t<=900); taps.push(now);
    if(taps.length>=5){ $("#debug-console")?.classList.toggle("hide"); taps=[]; dbg.push("Toggle overlay","ok"); }
  });

  // Journal & Debug (Réglages)
  $("#cb-debug-overlay")?.addEventListener("change", (e)=>{ $("#debug-console")?.classList.toggle("hide", !e.target.checked); });
  $("#btn-copy-logs")?.addEventListener("click", ()=> dbg.copy());
  $("#btn-clear-logs")?.addEventListener("click", ()=> dbg.clear());

  // RAZ & sauvegardes (Réglages — basique)
  $("#btn-raz-day")?.addEventListener("click", ()=>{
    S.today.counters={cigs:0,joints:0,beer:0,hard:0,liqueur:0}; persistTodayIntoHistory(); reflectCounters(); updateHeader(); saveState(S);
    dbg.push("RAZ du jour","ok");
  });
  $("#btn-raz-history")?.addEventListener("click", ()=>{
    S.history={}; persistTodayIntoHistory(); renderChart(_period); saveState(S); dbg.push("RAZ historique","ok");
  });
  $("#btn-raz-factory")?.addEventListener("click", ()=>{
    S=DefaultState(); saveState(S); hydrateUIFromState(); renderChart(_period); Cal?.update?.(S); dbg.push("RAZ réglages (usine)","ok");
  });

  $("#btn-save-json-settings")?.addEventListener("click", ()=>{
    const blob=new Blob([JSON.stringify(S,null,2)],{type:"application/json"});
    const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="stopaddict_settings.json"; document.body.appendChild(a); a.click(); a.remove();
    dbg.push("Sauvegarde JSON (réglages) ok","ok");
  });
  $("#file-import-json-settings")?.addEventListener("change", async (ev)=>{
    const f=ev.target.files?.[0]; if(!f) return;
    try{ const t=await f.text(); const obj=JSON.parse(t); S={...DefaultState(),...obj}; saveState(S); hydrateUIFromState(); renderChart(_period); Cal?.update?.(S); dbg.push("Import JSON (réglages) ok","ok"); }
    catch(e){ dbg.push("Import JSON (réglages) erreur: "+e?.message,"err"); alert("Import JSON invalide."); }
