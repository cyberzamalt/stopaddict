...
try {
  ...
  if (typeof m?.mountSettings === "function") mountSettings = m.mountSettings;
} catch {}

let Tips = null;
try {
  const m = await import('./tips.js');
  if (typeof m?.mountTips === "function") Tips = m;
} catch {}

let CalendarMod = null;
try {
  const m = await import('./calendar.js');
  if (typeof m?.mountCalendar === "function") CalendarMod = m;
} catch {}

// charge le module ressources (auto-mount interne)
try { await import('./resources.js'); } catch {}
...
function computeCost(counters=S.today?.counters||{}) {
  ...
  for (const k of KINDS){
    ...
    t += Number(counters[k]||0) * unitPrice(k);
  }
  return t;
}
...
function persistTodayIntoHistory(){
  S.history ??= {};
  S.today ??= { date: todayKey(), counters: { cigs:0,joints:0,beer:0,hard:0,liqueur:0 }, active: { cigs:true,joints:true,beer:true,hard:true,liqueur:true } };
  const key = todayKey();
  if (S.today.date !== key){
...
function renderChart(period=_period){
  ...
  if (period === "day") {
    ...
    counters = {
      ...
      cost: Number(d.cost||0), saved: Number(d.saved||0)
    };
    ...
    for (const k of Object.keys(S.history||{})){
      if (k>=range.start && k<=range.end){
        ...
        counters.cost    += Number(d.cost||0);
        counters.saved   += Number(d.saved||0);
      }
    }
...
function initStats(){
  ...
  $("#file-import-json")?.addEventListener("change", async (ev)=>{
    ...
    try{
      ...
      // merge simple, en conservant la forme attendue
      S = { ...DefaultState(), ...S, ...obj };
      saveState(S);
      hydrateUIFromState();
      renderChart(_period);
      Cal?.update?.(S);
...
function initHabits(){
  ...
  $("#btn-habits-save")?.addEventListener("click", ()=>{
    S.goals = {
      cigs:   Number($("#goal-cigs")?.value||0),
      joints: Number($("#goal-joints")?.value||0),
      beer:   Number($("#goal-beer")?.value||0),
      hard:   Number($("#goal-hard")?.value||0),
      liqueur:Number($("#goal-liqueur")?.value||0),
    };
    persistTodayIntoHistory();
    updateHeader();
    renderChart(_period);
  ...
  $("#btn-habits-reset")?.addEventListener("click", ()=>{
    ...
    S.goals = { ...def.goals };
    setVal("#goal-cigs",0);
    setVal("#goal-joints",0);
    setVal("#goal-beer",0);
    setVal("#goal-hard",0);
    setVal("#goal-liqueur",0);
    persistTodayIntoHistory();
    updateHeader();
    renderChart(_period);
    saveState(S);
...
(async function initApp() {
  ...
  S.today ??= { date: todayKey(), counters:{ cigs:0,joints:0,beer:0,hard:0,liqueur:0 }, active:{ cigs:true,joints:true,beer:true,hard:true,liqueur:true } };
  S.history ??= {};
  S.goals   ??= { cigs:0,joints:0,beer:0,hard:0,liqueur:0 };
  ...
  if (mountSettings){
    mountSettings({
      ...
      persistTodayIntoHistory, updateHeader, renderChart, reflectCounters, dbg
    });
  }
  ...
  // Debug overlay (tap x5 sur la date)
  let taps=[];
