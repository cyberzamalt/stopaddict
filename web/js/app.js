...
const dbg = {
  push(msg, type = "info") {
    ...
    S.debug?.logs?.push ? S.debug.logs.push(line) : (S.debug = { logs:[line] });
...
function unitPrice(kind) {
  const p = S.prices;
  const v = S.variants || { classic:{}, rolled:{}, cannabis:{}, alcohol:{ beer:{}, hard:{}, liqueur:{} } };
...
/* =========================================
   Réglages — settings.js si dispo, sinon fallback
   ========================================= */
...
function initSettingsFallback() {
  ...
  $("#btn-raz-factory")?.addEventListener("click", () => {
    ...
    hydrateUIFromState(); renderChart(); saveState(S);
...
/* =========================================
   Boot (sans top-level await bloquant)
   ========================================= */
async function boot() {
  // Imports dynamiques ici (pas au top-level)
  try {
    const modS = await import('./settings.js');
    if (typeof modS?.mountSettings === 'function') mountSettings = modS.mountSettings;
  } catch {}
  try {
    const modI = await import('./i18n.js');
    if (typeof modI?.initI18n === 'function') I18N = modI;
  } catch {}

  if (!S.today?.date) S.today = { ...(S.today||{}), date: todayKey() };
