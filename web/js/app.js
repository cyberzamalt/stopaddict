...
const dbg = {
  push(msg, type="info") {
    ...
    if (box && !box.classList.contains("hide")) {
      ...
      div.className = "debug-line"; div.textContent = line;
      box.appendChild(div); box.scrollTop = box.scrollHeight;
    }
  },
  clear(){ if (S.debug) S.debug.logs = []; $("#debug-console")?.replaceChildren(); },
...
function reflectCounters() {
  ...
  const setDis = (id, off) => { const el=$(id); if(!el) return; el.style.opacity = off ? "0.55" : "1"; el.style.pointerEvents = off ? "none" : "auto"; };
...
function initCounters() {
  ...
  for (const k of KINDS) {
    ...
    el.addEventListener("change", () => {
      ...
      reflectCounters(); saveState(S); Tips?.updateTips?.(S); Cal?.update?.(S);
    });
  }
  reflectCounters();
}
...
function updateHeader(){
  ...
  const cost = computeCost(); const saved = computeSaved();
  $("#hdr-cost").textContent  = fmtMoney(cost, S.currency);
  $("#hdr-saved").textContent = fmtMoney(saved, S.currency);
  const sum = KINDS.reduce((tot,k)=>tot+(S.today.counters[k]||0),0);
...
function dateRangeFor(period, ref=new Date()){
  const d=new Date(ref), start=new Date(d), end=new Date(d);
...
function renderChart(period=_period){
  ...
  let c = { cigs:0,joints:0,beer:0,hard:0,liqueur:0, cost:0, saved:0 };
  const now=new Date(), today=todayKey(now);
  if (period==="day"){
    const d=S.history[today]||{};
    c={ cigs:d.cigs||0,joints:d.joints||0,beer:d.beer||0,hard:d.hard||0,liqueur:d.liqueur||0,cost:d.cost||0,saved:d.saved||0 };
    $("#stats-date").textContent = new Date().toLocaleDateString("fr-FR");
  } else {
    const r=dateRangeFor(period,now);
    for (const k of Object.keys(S.history)) if (k>=r.start && k<=r.end) {
      const d=S.history[k]||{};
      c.cigs+=d.cigs||0; c.joints+=d.joints||0; c.beer+=d.beer||0; c.hard+=d.hard||0; c.liqueur+=d.liqueur||0; c.cost+=d.cost||0; c.saved+=d.saved||0;
    }
    $("#stats-date").textContent = `${fmtDate(r.start)} → ${fmtDate(r.end)}`;
  }
...
function initStats(){
  ...
  $("#btn-export-csv")?.addEventListener("click", () => {
    ...
    const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(blob), download:"stopaddict_stats.csv" });
  ...
  $("#btn-export-json")?.addEventListener("click", () => {
    ...
    const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(blob), download:"stopaddict_export.json" });
    document.body.appendChild(a); a.click(); a.remove();
...
function initHabits(){
  ...
  $("#btn-habits-save")?.addEventListener("click", ()=>{
    ...
    persistTodayIntoHistory(); updateHeader(); renderChart(_period); saveState(S); dbg.push("Objectifs quotidiens enregistrés","ok"); Cal?.update?.(S);
  });
  $("#btn-habits-reset")?.addEventListener("click", ()=>{
    ...
    persistTodayIntoHistory(); updateHeader(); renderChart(_period); saveState(S); dbg.push("Objectifs réinitialisés","ok"); Cal?.update?.(S);
  });
...
// i18n placeholder (si un i18n.js arrive plus tard)
async function initI18nIfAvailable(){ /* noop */ }
...
// Boot — pas de DOMContentLoaded car <script defer>
let Cal = null;
(async function initApp(){
  ...
  // Toggle debug (5 taps sur la date)
  let taps=[];
